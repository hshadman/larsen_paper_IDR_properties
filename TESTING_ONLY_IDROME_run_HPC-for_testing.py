...
for provided_seq_name in seq_name_dir_df.seq_name.values[0:2]:
...
hpc_computed_quantities = pd.DataFrame(zip(seq_name_list,fC_shape_shape,fA_shape_shape,
                ...
                bounded_fraction_size_shape_list),columns=['seq_name_list','fC_shape_shape','fA_shape_shape',
                                                          'mean_inst_Rs','mean_RSA','fC_size_shape','fA_size_shape','bounded_frac_size_shape']).copy()

hpc_computed_quantities.to_csv('TESTING_HPC_computed_fC_values_all.csv',index=False)
