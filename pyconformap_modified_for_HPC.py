...
class PyConforMap():
    ...
    def plot_protein_against_GW(self,protein_label,provided_color='magenta'):
        ...
        fig = plt.figure(1, figsize=(8.5,8))
        ...
        step_x=0.15#(min(x_polmodel_GW)-max(x_polmodel_GW))/original_n_divisions
        step_y=1.5#(min(y_polmodel_GW)-max(y_polmodel_GW))/original_n_divisions
        n_divisions_x=round((self.max_x_val-self.min_x_val)/step_x)
        
        n_divisions_y=round((self.max_y_val-self.min_x_val)/step_y)
        
        gridx = np.linspace(self.min_x_val, self.max_x_val, n_divisions_x)
        
        gridy = np.linspace(self.min_y_val, self.max_y_val, n_divisions_y)
        ...
        self.fA_value = np.count_nonzero(grid_protein)/np.count_nonzero(grid_GW)
